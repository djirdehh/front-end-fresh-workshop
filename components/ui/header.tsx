...
export default function Header() {
  ...
  return (
    <>
      {showBanner && (
        <div className="bg-indigo-600 text-white mb-2 py-1 px-4 text-center font-inter tracking-normal">
          <p className="text-[0.65rem] font-medium">
            <span className="md:hidden">🦖 subscribers get early-bird pricing — <span className="font-bold">$199 USD</span>!</span>
            <span className="hidden md:inline">Welcome from <span className="font-semibold">Building Large-Scale Web Apps</span> 🦖! Newsletter subscribers will continue to get early-bird pricing — <span className="font-bold">$199 USD</span>.</span>
          </p>
        </div>
      )}
      <header className={`absolute ${showBanner ? 'top-8' : 'top-4 md:top-6'} w-full z-30 pb-4 md:pb-6 border-b [border-image:linear-gradient(to_right,transparent,theme(colors.indigo.300/.4),transparent)1] dark:[border-image:linear-gradient(to_right,transparent,theme(colors.indigo.300/.16),transparent)1] shadow-[0_1px_0_0_theme(colors.white/.2)] dark:shadow-none`}>
